...
/*
gst-launch-1.0 -e \
    v4l2src device=/dev/video31 num-buffers=100 ! video/x-raw,format=NV12,width=800,height=600,framerate=30/1 ! videoconvert ! queue ! videomixer name=mix sink_0::xpos=0 sink_0::ypos=0 sink_1::xpos=800 sink_1::ypos=0 ! mpph264enc ! h264parse ! mp4mux reserved-moov-update-period=10 ! filesink location=/tmp/output.mp4 \
...
gcc my-camera-confluence-mpp.c -o my-camera-confluence-mpp ` pkg-config --cflags --libs gstreamer-1.0 `
*/
int main(int argc, char *argv[]) {
  GstElement *pipeline, *source1, *source2, *filter1, *filter2, *converter1, *converter2, *queue1, *queue2, *mixer, *encoder, *parser, *muxer, *sink;
  GstCaps *caps;
  GstBus *bus;
  GstMessage *msg;
  ...
  if (!pipeline || !source1 || !source2 || !filter1 || !filter2 || !converter1 || !converter2 || !queue1 || !queue2 || !mixer || !encoder || !parser || !muxer || !sink) {
    g_printerr ("Not all elements could be created.\n");
    return -1;
  }

  /* Build the pipeline */
  gst_bin_add_many (GST_BIN (pipeline), source1, filter1, converter1, queue1, mixer, encoder, parser, muxer, sink, source2, filter2, converter2, queue2, NULL);
  
  if (!gst_element_link_many (source1, filter1, converter1, queue1, mixer, encoder, parser, muxer, sink, NULL)) {
